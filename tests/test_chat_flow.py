...
class ChatFlowTest(unittest.TestCase):
    ...
    def test_ask_morn_question_prefers_doc_grounded_answer_without_model_call(self):
        original_vector_search = main.vector_search
        original_gemini_generate = main.gemini_generate
        original_drive_store = getattr(main, "drive_store_v2", None)
        ...
        class FakeDriveStore:
            def __init__(self):
                self.docs = [
                    main.WorldDocInfo(folder="02 Sessions", title="Dossier Morna - sprawa Black Eel", doc_id="doc-morn", path_hint="02 Sessions/Dossier Morna - sprawa Black Eel", entity_type=main.WorldEntityType.session),
                    main.WorldDocInfo(folder="02 Sessions", title="Krew Na Gwiazdach - Rozdzial 1 - Cienie w Port Peril", doc_id="doc-r1", path_hint="02 Sessions/Krew Na Gwiazdach - Rozdzial 1 - Cienie w Port Peril", entity_type=main.WorldEntityType.session),
                    main.WorldDocInfo(folder="01 Bible", title="Przewodnik po Shackles", doc_id="doc-shackles", path_hint="01 Bible/Przewodnik po Shackles", entity_type=main.WorldEntityType.other),
                ]
            ...
            def read_doc(self, doc_ref):
                if doc_ref.title == "Dossier Morna - sprawa Black Eel":
                    return (
                        "1. Rdzen sprawy\n"
                        "Morn zalegalizowal falszywa korekte manifestu i zajecie dlugu wobec Black Eel. "
                        "To byl papierowy rabunek.\n\n"
                        "2. Ladunek Black Eel\n"
                        "Ladunek obejmowal kore febrowa, zywice alchemiczna i dwie skrzynie oznaczone jako medyczne z Mwangi.\n\n"
                        "3. Co trzymal Morn i dlaczego stal sie celem\n"
                        "Morn zachowal druga kopie dokumentow jako polise i stal sie wygodnym kozlem ofiarnym.\n\n"
                        "4. Co jest w pakiecie u Halvine\n"
                        "W pakiecie sa strony z Ksiegi Przysiag, pierwotny manifest i korekta manifestu."
                    )
                if doc_ref.title == "Krew Na Gwiazdach - Rozdzial 1 - Cienie w Port Peril":
                    return (
                        "1. Streszczenie dla MG\n"
                        "Bohaterowie trafiaja do Port Peril w Shackles. "
                        "W trakcie wydarzenia dochodzi do zamachu na Tavina Morna.\n"
                    )
                if doc_ref.title == "Przewodnik po Shackles":
                    return (
                        "1. Czym Shackles jest na co dzien\n"
                        "To archipelag portow, wysp i kapitanow.\n\n"
                        "2. Jak wygladaja porty i ulice\n"
                        "Porty sa brudne, wilgotne i pelne drewna, soli i przemocy."
                    )
                return ""

        try:
            main.vector_search = lambda question, top_k: (_ for _ in ()).throw(AssertionError("vector_search should not be called"))
            main.gemini_generate = lambda prompt, **kwargs: (_ for _ in ()).throw(AssertionError("gemini_generate should not be called"))
            main.drive_store_v2 = FakeDriveStore()
            response = main.ask(
                main.AskRequest(
                    question="CoĹ› o sprawie Morna w Shackles?",
                    include_sources=True,
                )
            )
        finally:
        ...
        self.assertIn("Black Eel", response.answer)
        self.assertTrue(any(source["title"] == "Dossier Morna - sprawa Black Eel" for source in response.sources))

    def test_ask_first_part_question_prefers_doc_grounded_answer_without_model_call(self):
        original_vector_search = main.vector_search
        original_gemini_generate = main.gemini_generate
        original_drive_store = getattr(main, "drive_store_v2", None)
        ...
        class FakeDriveStore:
            ...
            def read_doc(self, doc_ref):
                if doc_ref.title == "Krew Na Gwiazdach - Rozdzial 1 - Cienie w Port Peril":
                    return (
                        "1. Streszczenie dla MG\n"
                        "Bohaterowie trafiaja do Port Peril w Shackles w noc publicznej aukcji lupow i przyslug. "
                        "Dochodzi do zamachu na Tavina Morna.\n\n"
                        "2. Tlo i motywy\n"
                        "Prawo w Shackles to umowa i przemoc. "
                        "W cieniu dzialaja Kavo i Elvire.\n\n"
                        "3. Postacie\n"
                        "Marcellus Vhal\n"
                        "Tavin Morn\n"
                        "Captain Liora Haldrin\n"
                    )
                if doc_ref.title == "Przewodnik po Shackles":
                    return (
                        "1. Czym Shackles jest na co dzien\n"
                        "To archipelag portow, wysp i kapitanow.\n\n"
                        "2. Jak wygladaja porty i ulice\n"
                        "Porty sa brudne, wilgotne i pelne pracy, hazardu i przemocy."
                    )
                return ""
        ...
        try:
            ...
            response = main.ask(
                main.AskRequest(
                    question="Masz informacje na temat pierwszej czesci kampanii? Podaj je.",
                    include_sources=True,
                )
            )
        finally:
        ...
        self.assertIn("Rozdzial 1", response.answer)
        self.assertIn("Port Peril", response.answer)
        self.assertTrue(any(source["title"] == "Krew Na Gwiazdach - Rozdzial 1 - Cienie w Port Peril" for source in response.sources))

    def test_ask_campaign_overview_prefers_doc_grounded_answer_without_model_call(self):
        original_vector_search = main.vector_search
        original_gemini_generate = main.gemini_generate
        original_drive_store = getattr(main, "drive_store_v2", None)
        ...
        class FakeDriveStore:
            ...
            def read_doc(self, doc_ref):
                texts = {
                    "Campaign Bible": (
                        "1. Premisa\n"
                        "Bohaterowie zostaja wciagnieci w polityczna ukladanke przed epoka glodu i zimna.\n\n"
                        "3. Centralny konflikt\n"
                        "Stare porzadki scieraja sie z nowa koniecznoscia."
                    ),
                    "Przewodnik po Shackles": (
                        "1. Czym Shackles jest na co dzien\n"
                        "To archipelag portow, wysp i kapitanow.\n\n"
                        "2. Jak wygladaja porty i ulice\n"
                        "Porty sa brudne, wilgotne i pelne przemocy."
                    ),
                    "Krew Na Gwiazdach - Rozdzial 1 - Cienie w Port Peril": (
                        "1. Streszczenie dla MG\n"
                        "Bohaterowie trafiaja do Port Peril. Dochodzi do zamachu na Tavina Morna."
                    ),
                    "Dossier Morna - sprawa Black Eel": (
                        "1. Rdzen sprawy\n"
                        "Sprawa Black Eel to papierowy rabunek zalegalizowany przez Morna."
                    ),
                }
                return texts.get(doc_ref.title, "")
        ...
        self.assertIn("Shackles", response.answer)
        self.assertIn("Port Peril", response.answer)
        self.assertTrue(any(source["title"] == "Campaign Bible" for source in response.sources))
